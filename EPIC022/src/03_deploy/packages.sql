...
--epic022 - paquetes de transacciones del cliente
spool ./src/03_deploy/packages/packages_loop.sql;

prompt var codclavecic number;

begin
  for target_pointer in (select distinct codclavecic, codunicocli, periodo as codmes from tmp_clinuevo_alertas_1)
  loop
...
/*
--epic022 - paquetes de eecc del cliente
...
begin
  ...
  loop
    ...
    dbms_output.put_line('define codmes = '''||target_pointer.codmes||''';');
    dbms_output.put_line('exec :codclavecic := '''||target_pointer.codclavecic||'''; ');
    dbms_output.put_line('@@./src/03_deploy/packages/paquetes_eecc.sql;');
  end loop;
