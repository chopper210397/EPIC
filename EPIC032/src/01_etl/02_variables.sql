...
truncate table tmp_epic032_mto_semana;
insert into tmp_epic032_mto_semana 
select distinct 
...
truncate table tmp_epic032_mto_ctd_semanal;
insert into tmp_epic032_mto_ctd_semanal 
with tmp_autt007 as(
select 
    a.*,
    round((a.monto_total/b.monto_total)*100,2) as porcentaje_monto,
    round((a.ctd_trx/b.ctd_trx)*100,2) as porcentaje_ctd
from tmp_epic032_mto_semana a
left join tmp_epic032_monto_total b on a.periodo=b.periodo 
...
truncate table tmp_epic032_monto_total;
insert into tmp_epic032_monto_total 
select distinct 
...
truncate table tmp_epic032_pre_perfil;
insert into tmp_epic032_pre_perfil 
select  
...
inner join tmp_epic032_docggtt e on a.nbrclibeneficiario=e.nbrclibeneficiario
...
where
a.fecdia between trunc(add_months(sysdate,:intervalo_1-6),'mm') and trunc(last_day(add_months(sysdate,:intervalo_1-1))) 
and a.codproducto in ('TRAXRE') 
and a.codtipestadotransaccion = '04' ;

-- Obtencion del codunicoli limpio
truncate table tmp_epic032_perfil;
insert into tmp_epic032_perfil 
select 
    a.*,f.codunicocli_limpio 
from tmp_epic032_pre_perfil a
left join tmp_epic032_univ f on a.nbrclibeneficiario=f.nbrclibeneficiario;

-- Obtenemos el monto total de la base de perfil
truncate table tmp_epic032_group_perfil;
insert into tmp_epic032_group_perfil 
select 
    to_number(to_char(fecdia,'yyyymm')) as periodo,codunicocli_limpio,
    sum(mtotransacciondolar) as mto_total 
from tmp_epic032_perfil
group by to_number(to_char(fecdia,'yyyymm')),codunicocli_limpio;

-- Obtenemos la base de los clientes con sus montos para los diferentes meses
truncate table tmp_epic032_perfil1;
insert into tmp_epic032_perfil1 
select a.* from tmp_epic032_group_perfil a
union all
select periodo,codunicocli_limpio,monto_total from tmp_epic032_monto_total;

--Obtenemos las fechas de los 6 meses anteriores
truncate table tmp_epic032_fec_perfil;
insert into tmp_epic032_fec_perfil 
select  a.*,
...
from tmp_epic032_monto_total a;

-- Obtenemos el promedio y la desviacion estandar  
truncate table tmp_epic032_fec_perfil2;
insert into tmp_epic032_fec_perfil2 
select 
    a.periodo,a.codunicocli_limpio,avg(nullif(b.mto_total,0)) as media_depo, 
    stddev(nullif(b.mto_total,0)) as desv_depo
from tmp_epic032_fec_perfil a 
left join tmp_epic032_perfil1 b on a.codunicocli_limpio = b.codunicocli_limpio
...
truncate table tmp_epic032_ordenantes;
insert into tmp_epic032_ordenantes 
select distinct 
    to_number(to_char(fecdia,'yyyymm')) as periodo,codunicocli_limpio,
...
-- Variable ros
truncate table tmp_epic032_ros;
insert into tmp_epic032_ros 
with tmp_epic032 as (
select distinct
    a.fecdia,
...
-- Variables LSB/NP
truncate table tmp_epic032_pre_lsb;
insert into tmp_epic032_pre_lsb 
select distinct
    a.fecdia,
    a.codunicocli_limpio,'0000'||''||'GR'||codmatricula||'7' as codunicocli_gremio 
from tmp_epic032_univ a
...
truncate table tmp_epic032_aux_lsb;
insert into tmp_epic032_aux_lsb 
select distinct
    a.fecdia,codunicocli_limpio,
    case when codunicocli_gremio = '0000GR7' 
        then codunicocli_limpio else codunicocli_gremio end as codunicocli
from tmp_epic032_pre_lsb a ;

truncate table tmp_epic032_lsb;
insert into tmp_epic032_lsb 
select distinct to_number(to_char(fecdia,'yyyymm')) as periodo,codunicocli_limpio  
from tmp_epic032_aux_lsb a
...
-- Variable AN
truncate table tmp_epic032_pre_np;
insert into tmp_epic032_pre_np 
select distinct 
    to_number(to_char(a.fecdia,'yyyymm')) as periodo,a.codunicocli_limpio,b.codclavecic,a.fecdia
from tmp_epic032_univ a
inner join ods_v.md_clientenegativo b on b.codunicocli=a.codunicocli_limpio
where b.tipestclinegativo <> 'H';

truncate table tmp_epic032_an;
insert into tmp_epic032_an 
select distinct periodo,a.codunicocli_limpio 
from tmp_epic032_pre_np a
left join ods_v.md_motivodetalleclinegativo c on c.codclavecic = a.codclavecic
where c.tipmotivonegativo in ('013')
and a.fecdia>c.fecregistrodetallenegativo;

-- Obtencion de los campos de apellidos y nombres del beneficiario y ordenante 
truncate table tmp_epic032_nombre;
insert into tmp_epic032_nombre 
select distinct 
...
truncate table tmp_epic032_varfamiliares;
insert into tmp_epic032_varfamiliares 
with tmp_epic032 as (
select a.*,
case
...
truncate table tmp_epic032_cliente;
insert into tmp_epic032_cliente 
select distinct 
    a.codunicocli_limpio 
from tmp_epic032_univ a
...
-- Tablon final
truncate table tmp_epic032_univ_total;
insert into tmp_epic032_univ_total  
select distinct     
    a.*,
...
from tmp_epic032_monto_total a
left join tmp_epic032_ordenantes b on a.periodo=b.periodo and a.codunicocli_limpio=b.codunicocli_limpio
left join tmp_epic032_extr c on a.codunicocli_limpio=c.codunicocli_limpio
left join tmp_epic032_ros d on a.periodo = d.periodo and a.codunicocli_limpio=d.codunicocli_limpio
left join tmp_epic032_lsb e on a.periodo = e.periodo and a.codunicocli_limpio=e.codunicocli_limpio
left join tmp_epic032_an f on a.periodo = f.periodo and a.codunicocli_limpio=f.codunicocli_limpio
left join tmp_epic032_fec_perfil2 h on a.periodo = h.periodo and a.codunicocli_limpio=h.codunicocli_limpio
left join tmp_epic032_varfamiliares i on a.periodo = i.periodo and a.codunicocli_limpio=i.codunicocli_limpio
left join tmp_epic032_cliente j on a.codunicocli_limpio=j.codunicocli_limpio
left join tmp_epic032_mto_ctd_semanal k on a.periodo = k.periodo and a.codunicocli_limpio=k.codunicocli_limpio;
