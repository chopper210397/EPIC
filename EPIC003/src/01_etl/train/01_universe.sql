...
WHERE 
 ...
 AND GGT.FECDIA  BETWEEN TRUNC(ADD_MONTHS(SYSDATE,-7),'MM') AND TRUNC(LAST_DAY(ADD_MONTHS(SYSDATE,-1)))
...
FROM ODS_V.HD_DOCUMENTOEMITIDOGGTT GGT 
     ...
     LEFT JOIN ODS_V.HD_TIPOCAMBIOSALDODIARIO TP ON TP.CODMONEDA=GGT.CODMONEDA AND TP.FECTIPCAMBIO=GGT.FECDIA
WHERE 
      GGT.FECDIA  BETWEEN TRUNC(ADD_MONTHS(SYSDATE,-13),'MM') AND TRUNC(LAST_DAY(ADD_MONTHS(SYSDATE,-8)));
...
FROM ODS_V.HD_MOVIMIENTOBANNI  A
	 ...
	 LEFT JOIN TMP_TTEE_CLIENTE_SEGEMENTO E ON E.CODSUBSEGMENTO=CL.CODSUBSEGMENTO
	 LEFT JOIN ODS_V.UD_SECTORISTA SC ON TRIM(SC.CODSECTORISTA) = TRIM(CL.CODSECTORISTA)
WHERE  B.FECDIA  BETWEEN TRUNC(ADD_MONTHS(SYSDATE,-7),'MM') AND TRUNC(LAST_DAY(ADD_MONTHS(SYSDATE,-1)))
	AND DTC.DESTIPPER = 'NATURAL'
...
WHERE 
     B.FECDIA  BETWEEN TRUNC(ADD_MONTHS(SYSDATE,-13),'MM') AND TRUNC(LAST_DAY(ADD_MONTHS(SYSDATE,-8)));
