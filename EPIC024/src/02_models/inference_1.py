...
class Inference:
    ...
    def load_data(self):
        ...
        try:
            ...
            dat=dataframe[(dataframe.TIPPER=='E')] #Juridica
    ...
    def feature_engineering(self, dataframe):
        ...
        try:
            ...
            logging.info("Creando el archivo 02_preprocessed_ppjj.csv")
            with open(r'./data/production/02_preprocessed_ppjj.csv', 'w+',
                      encoding='latin-1',newline="") as csv_file:
                dataframe.to_csv(path_or_buf=csv_file, sep='|', index=False)
            logging.info("Guardamos el archivo 02_preprocessed_ppjj.csv")
        except ValueError:
            logging.error(
    ...
    def rule_1(self):
        ...
        try:
            ...
            dataset = pd.read_csv(r'./data/production/02_preprocessed_ppjj.csv',encoding='unicode_escape',  sep="|")
            logging.info("Filtrando el archivo 02_preprocessed_ppjj.csv")
            # Identifica a los clientes para la segmentación escalon_3
            df_test_sc3 = dataset.loc[dataset['MTO_INGRESOS_MES']>30e+6]
    ...
    def rule_2(self):
        ...
        try:
            #Lectura del archivo preprocesado
            dataset = pd.read_csv(r'./data/production/02_preprocessed_ppjj.csv',encoding='unicode_escape', sep="|")
            dataset_f=dataset[(dataset['MTO_INGRESOS_MES']>=1100) & (dataset['MTO_INGRESOS_MES']<=30e+6)]
            df_2 = dataset_f[['FLG_PERFIL_INGRESOS_3DS', 'MTO_CONOTROSPROXIMOS', 'CTD_CONOTROSPROXIMOS']]
            with open(r'./src/02_models/iforest_model_ppjj', "rb") as f:
    ...
    def output(self, dataset_escalon2,dataset_escalon3):
        ...
        try:
            ...
            df_final[vars_export].to_csv(r'./data/production/04_predicted_ppjj.csv', index=False, sep='|')
        except Exception:
            exc_type, exc_value, exc_traceback = sys.exc_info()
            logging.warning("Sucedio un error inesperado: "+
                            exc_type+" - "+exc_value+" - "+exc_traceback)
